...
CREATE TABLE checkins (
  ...
  duration_min INTEGER GENERATED ALWAYS AS (
    ...
    THEN EXTRACT(EPOCH FROM (checked_out - checked_in))/60::INTEGER
    ELSE NULL END
  ) STORED
);
...
CREATE TABLE anomalies (
  ...
  UNIQUE(gym_id, type) DEFERRABLE INITIALLY IMMEDIATE -- for idempotency constraint, wait user didn't mention this explicitly in schema, they said "Uses upsert with a unique constraint on (gym_id, type) where resolved = false".
);

-- To fulfill "unique constraint on (gym_id, type) where resolved = false":
CREATE UNIQUE INDEX idx_anomalies_unique_active ON anomalies (gym_id, type) WHERE resolved = FALSE;
